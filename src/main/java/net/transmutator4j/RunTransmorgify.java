...
public class RunTransmorgify implements Runnable{
	...
	public RunTransmorgify(String nameOfTestSuite, Pattern classesToMutate,
			File xmlFile) {
		super();
				...
				System.out.printf("mutating %s%n", classToMutate);
					...
					while(!done){
						...
						JavaProcessBuilder builder = new JavaProcessBuilder(
								...
								"net.transmutator4j.Transmorgify", 
								classToMutate,
								nameOfTestSuite,
								mutationCount+"",
				       ...
				       try {
				    	   ...
				    	   TransmorgifyUtil.EXIT_STATES exitState = TransmorgifyUtil.EXIT_STATES.getValueFor(exitValue);
				    	   if(exitState ==TransmorgifyUtil.EXIT_STATES.NO_MUTATIONS_MADE){
				    		   done = true;
				    		   System.out.println();
				    	   }
				    	   else {
				    		   ...
				    		   if(exitState == TransmorgifyUtil.EXIT_STATES.TIMED_OUT){
			    				   numberOfMutationsThatTimedOut++;
			    			   }
				    		   else if (exitState ==TransmorgifyUtil.EXIT_STATES.TESTS_ALL_STILL_PASSED){
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, IllegalArgumentException, SecurityException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, InstantiationException {
        ...
        try {
			...
			new RunTransmorgify(nameOfTestSuite, classesToMutates,xmlFile).run();
		
		} catch (ParseException e1) {
			HelpFormatter formatter = new HelpFormatter();
			formatter.printHelp( "RunTransmorgify -src <src> -test <unit test> [-out <xml file>]", options );
		}
	}
