...
public class Transmorgify {
	 ...
	 */
	public Transmorgify(JUnitCore junitCore, String testClass) throws ClassNotFoundException, IllegalArgumentException, SecurityException, IllegalAccessException, InvocationTargetException, NoSuchMethodException{
	...
	public static void main(String[] args){
		...
		cr.accept(adapter, 0);
			...
			final MutatedClassLoader classLoader = new MutatedClassLoader(Transmorgify.class.getClassLoader(),
					new ClassPathClassRepository(), classToMutate, compiled);
			if(adapter.hasMutated()){
			
				
				Class<?> c =Class.forName("net.transmutator4j.Transmorgify",false, classLoader);
				...
				pw.append(String.format(
						...
						TransmorgifyUtil.xmlEncode(adapter.getMutation().description()),
						failCount==0,
						testsStillPass? "Passed": "Failed"));
				pw.flush();
				pw.close();
				if(testsStillPass){
					TransmorgifyUtil.EXIT_STATES.TESTS_ALL_STILL_PASSED.exitSystem();
				}
				TransmorgifyUtil.EXIT_STATES.TESTS_FAILED.exitSystem();
			}
		...
		TransmorgifyUtil.EXIT_STATES.NO_MUTATIONS_MADE.exitSystem();
